...
impl User {
    ...
    pub fn visit_doctor(&mut self, measurements: Measurements) -> HealthReport {
        ...
        let blood_pressure_change = self.last_blood_pressure.map(|prev_bp| {
            (
                measurements.blood_pressure.0 as i32 - prev_bp.0 as i32,
                measurements.blood_pressure.1 as i32 - prev_bp.1 as i32,
            )
        });
...
fn main() {
    ...
    println!(
        "{} посетил врача {} раз. Изменение роста: {}, Изменение давления: {:?}",
        report.patient_name, report.visit_count, report.height_change, report.blood_pressure_change
    );
}
